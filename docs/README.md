...
The module and persisted-schema tables in [architecture.md](architecture.md)
are generated by `npm run sync:architecture`; add `-- --check` to check drift
without writing. Module paths come from `bin/` and `src/`; their authored
responsibilities and important dependencies live in
[architecture-modules.json](architecture-modules.json). Adding or removing a
module requires updating that description catalog. Schema families, readable
and written versions, stores, and scopes come from `RECORD_FAMILIES` in
`src/schemas.js`. Surrounding prose and command-output descriptions remain
hand-written. `test/architecture-tables.test.js` checks agreement in the ordinary
suite and in CI static checks, including documentation-only changes.
