...
| Correction result | [FORCED_SESSION_TEST_RESULTS.md](FORCED_SESSION_TEST_RESULTS.md) |
...
Open a new PowerShell 7 window in the exact checkout under test and run:
...
Shutdown acknowledges the worker only after the Git child emits `close`, which
means its stdio has drained. Graceful EOF remains the normal path. If it does
not complete within the fixed deadline, Windows termination targets the exact
Git process tree before the final bounded worker fallback.
