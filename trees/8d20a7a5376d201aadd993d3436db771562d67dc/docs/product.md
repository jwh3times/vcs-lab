...
| Product baseline | v0.16.0 release |
...
**Partial**, **Planned**, or **Deferred** at the v0.16.0 release baseline.
...
| FR-PLAN-08 | P2 | Plans shall expose a versioned portable proof bundle suitable for a remote verifier. | Implemented | `vlab proof-bundle` emits `vcs-lab.proof-bundle/v2` carrying the coverage evidence, not only the verdict; `vlab verify-proof` re-derives every classification from that evidence with its own copy of the lattice, and compares the evidence against the repository when it has one. Since v0.15.0 the bundle also carries Git's own bindings for what it claims ([ADR-0031](adr/0031-carry-a-bound-source-inventory-for-portable-verification.md), built in #36): the raw commit objects of the source range, reachability paths for every positive claim, receipt inclusion proofs against the notes tip, and the anchors they terminate at. A verifier without the repository recomputes each object id from its bytes, which is what makes the remote half sound — every adversarial shape #46 found now fails offline even after the bundle hash is restated. Conclusions carry a tier, and `new` and `candidate-equivalent` are reported as claimed rather than proven, because absence needs the repository. |
...
| FR-SPEC-05 | P0 | Sidecars shall avoid duplicating titles, positions, content hashes, and ordinary IDs. | Implemented | Sparse manifest v4 stores identity and exceptions only. |
...
| FR-PROTO-06 | P2 | Remote synchronization shall advertise capabilities and negotiate schema versions. | Advertising and negotiation implemented offline; the gateway is deferred | [ADR-0033](adr/0033-advertise-capabilities-as-a-document-negotiated-offline.md), built in #37. `vlab capabilities` projects `vcs-lab.capabilities/v1` from the runtime registries, and `--against` negotiates against a peer document or an envelope manifest. Negotiation is a pure function of two documents, so every conclusion a gateway would enable is already reachable without one. The gateway — serving the current document of a party not in the room — remains unscheduled and is the only part that needs a server. |
...
| Stable Markdown entities | Complete for parser v2; historical v1 reads retained | Edit/move and migration tests |
| Deterministic Markdown merge | Complete for section-level rules | Clean and blocked merge tests |
| Sparse metadata and incremental indexing | Complete for manifest v4 | Corpus, migration, zero-read tests |
...
do not establish a need for a new canonical store. Partial pilot evidence must
remain visible without treating a suite fixture or a single session as a user
preference, sustained improvement, or reproduction rate.
